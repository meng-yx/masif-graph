...
MODELS=/work/upthomae/Meng/phase5_af3/models
PY=/work/upthomae/Meng/conda_envs/masif-graph/bin/python
LOGDIR=/scratch/ymeng/masif-graph/logs/phase3/af3_surf
mkdir -p "$LOGDIR" "$RAW"
log="$LOGDIR/${id}.log"
...
for C in "$C1" "$C2"; do
  ...
  cif=$(find "$MODELS/$name" -name "${name}_model.cif" 2>/dev/null | head -1)
